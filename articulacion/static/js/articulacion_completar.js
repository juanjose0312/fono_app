...
function FuncionBoton(idbutton, idstatus, idresultado, letra) {
    if (!isRecording) {
        // Iniciar grabación
        navigator.mediaDevices.getUserMedia({ audio: true }) // Solicitar acceso al micrófono
            .then(stream => {
                ...
                mediaRecorder.addEventListener('stop', () => {
                    ...
                    formData.append('idbutton', idbutton); // Añadir el id del botón al FormData

                    // Enviar el FormData al servidor usando fetch
                    fetch(`http://127.0.0.1:8000/articulacion/articulacion_completar/${letra}/`, {
...
for (let i = 1; i <= 7; i++) {
    const button = document.getElementById(`recordButton_${i}`);
    const letra = button.getAttribute('data-letra');
    button.addEventListener('click', () => {
        FuncionBoton(`recordButton_${i}`, `status_${i}`, `resultado_${i}`, letra);
    });
}
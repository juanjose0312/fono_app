    ...
    if (!isRecording) {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true }) // Solicitar acceso al micrófono
            .then(stream => {
                ...
                document.getElementById('status').textContent = 'Grabando...'; // Actualizar el texto de estado
                document.getElementById('recordButton').textContent = 'Stop Recording'; // Cambiar el texto del botón
                ...
                mediaRecorder.addEventListener('stop', () => {
                    ...
                    .then(data => {
                        document.getElementById('resultado').textContent = `Resultado: ${data.result}`; // Mostrar el resultado del reconocimiento de voz en el elemento con id 'resultado'
                    })
                    .catch(error => console.error('Error:', error)); // Mostrar cualquier error que ocurra durante la solicitud en la consola

                    document.getElementById('status').textContent = 'Grabación completa.'; // Actualizar el texto de estado
                    document.getElementById('recordButton').textContent = 'Start Recording'; // Cambiar el texto del botón
